...
class QRNN(torch.nn.Module):
    ...
    def forward(self, input, hidden=None):
        if not self.bidirectional:
            ...
            for i, layer in enumerate(self.layers):
                input, hn = layer(input, None if hidden is None else hidden[i])
                next_hidden_1.append(hn)
                if self.dropout != 0 and i < len(self.layers) - 1:
                    input = torch.nn.functional.dropout(input, p=self.dropout, training=self.training, inplace=False)
            next_hidden_1 = torch.cat(next_hidden_1, 0).view(self.num_layers, *next_hidden_1[0].size()[-2:])
            # (seq_len, batch, input_size)

            next_hidden_2 = []
            # input2 = torch.flip(input2, [0])

            for i, layer in enumerate(self.layers_2):
                input2, hn = layer(input2, None if hidden is None else hidden[i])
                next_hidden_2.append(hn)
                if self.dropout != 0 and i < len(self.layers) - 1:
                    input2 = torch.nn.functional.dropout(input2, p=self.dropout, training=self.training, inplace=False)
            next_hidden_2 = torch.cat(next_hidden_2, 0).view(self.num_layers, *next_hidden_2[0].size()[-2:])

            return torch.cat((input, input2), 2).cuda(), torch.cat((next_hidden_1, next_hidden_2), 2).cuda()
...
if __name__ == '__main__':
    ...
    qrnn = QRNN(input_size, hidden_size, num_layers=2, dropout=0.4, bidirectional=True)
    qrnn.cuda()
